...
from etl_inventory.extract import extract_stock_movements
import logging
import pandas as pd
...
set_last_raw_ts_sql = Path(SCRIPT_DIR / "sql/set_last_raw_ts.sql").read_text(encoding="utf-8")
...
def extract_incremental_data(source, store_id, start_timestamp, ):
    ...
    return extract_stock_movements(source, store_id, batch_dates, SCRIPT_DIR)

def insert_ignore(table, conn, keys, data_iter):
    # Pandas gives us a SQLTable, not a SQLAlchemy Table
