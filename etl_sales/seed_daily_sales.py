...
create_fact_daily_sales_sql = Path(SCRIPT_DIR / "db/create_fact_daily_sales.sql").read_text(encoding="utf-8")
extract_insert_daily_sales_sql = Path(SCRIPT_DIR / "db/extract_insert_daily_sales.sql").read_text(encoding="utf-8")
...
def main():
    ...
    logging.info(f"Seeding daily sales data")
    
    with analytics_engine.begin() as conn:
        conn.execute(text(extract_insert_daily_sales_sql))
    
    logging.info(f"Daily sales data seed complete")
